...
def room_booking_chart(request):
    room_sizes = Room.objects.values('size').annotate(count=Count('id')).order_by('size')

    df = {
        "sizes": [r["id"] for r in room_sizes],
        "counts": [r["rating"] for r in room_sizes]
    }

    fig = px.bar(df, x="sizes", y="counts", title="Room Size Distribution")
    graph_html = pio.to_html(fig, full_html=False)

    return render(request, "room_booking/room_list.html", {"graph_html": graph_html})

def room_booking_chart_data(request):
    rooms = Room.objects.all()

    data = {
        "names": [room.name for room in rooms],
        "beds": [room.beds for room in rooms],
        "baths": [room.baths for room in rooms],
        "sizes": [room.size for room in rooms],
        "ratings": [room.rating for room in rooms],
        "reviews_count": [room.reviews_count for room in rooms],
